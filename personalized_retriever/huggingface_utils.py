...
class PreprocessFunction:
    ...
    def __init__(self, tokenizer, review_history, do_retrieval=False, do_evidence=False, relevance_threshold=0.55):
        self.tokenizer = tokenizer 
        self.review_history = review_history
        self.do_retrieval = do_retrieval
        self.do_evidence = do_evidence
        self.relevance_threshold = relevance_threshold

    def __call__(self, example):

        # Tokenize the texts
        result = self.tokenizer(example['text'], 
                                padding=False, 
                                max_length=20, 
                                truncation=True
                               )
        ...
        if self.do_retrieval or self.do_evidence:
            cos = torch.nn.CosineSimilarity(dim=1, eps=1e-6)
            cosine_sim = cos(
                torch.from_numpy(
                    self.review_history.get_ui_review(example['user'], example['item'], return_embedding=True)
                ), 
                torch.from_numpy(self.review_history.text_table[review_history_input_ids])
            )
            
            if self.do_retrieval:
                result['retrieval_label'] = (cosine_sim > self.relevance_threshold).long().tolist()
                # print(sum((cosine_sim > self.relevance_threshold).long().tolist()))
                # print((cosine_sim > self.relevance_threshold).long().tolist())
                
            if self.do_evidence:
                evidence_text = self.review_history.raw_text_table[
                    review_history_input_ids[torch.argmax(cosine_sim).item()]
                ]
                evidence_result = self.tokenizer(evidence_text, padding=False, max_length=20, truncation=True)
                
                for key, value in self.tokenizer('evidence#').items():
                    evidence_result[key] = value+evidence_result[key]
                for key, value in self.tokenizer(self.tokenizer.eos_token).items():
                    evidence_result[key] = evidence_result[key]+value 
                
                for evidence_key in evidence_result:
                    result['evidence_'+evidence_key] = evidence_result[evidence_key]        
        return result
...
class CustomTrainer(Trainer):
    ...
    def prediction_step(self,model,inputs,prediction_loss_only, ignore_keys):
        ...
        return (output.loss.detach().cpu(), output.regression_loss, output.lm_loss)
    ...
    def evaluation_loop(
            ...
            for step, inputs in enumerate(dataloader):
                loss, regression_loss, ret_loss = self.prediction_step(
                    model, 
                    inputs, 
                    prediction_loss_only, 
                    ignore_keys=ignore_keys
                )
                # print(loss, regression_loss, ret_loss)
            ...
            metrics = {
                ...
                'regression_loss':np.mean(regression_loss_host)
            }
