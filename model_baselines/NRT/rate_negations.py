"""
Module to let PEPLER rate nagations generated by
...
def main(args):
    ...
    i = 0
    for sample in tqdm(positive_samples[:n_samples]):
        out_string = out_string+'\n## '+str(i+1)+'\\'+str(n_samples)+' ##\n'
        user, item, rating, seq = sample['user'], sample['item'], sample['rating'], predictor.seq2language(sample['text'])
        ppl = predictor.ppl(user, item, seq)
        seq_hat = generate_batch_negation([seq])[0]
        ppl_hat = predictor.ppl(user, item, seq_hat)
