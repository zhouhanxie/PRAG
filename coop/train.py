...
class Trainer:
        ...
        while True:
            for batch in train:
                ...
                if self.global_step % self.checkout_step == 0:
                    ...
                    p.close()
                    self._evaluate()
                    if isinstance(self.model, BiMeanVAE) and self.global_step == 1000:
    ...
    def _finalize(self):
        print('finalized is overriden, for now just manually select the best model')
    ...
    def _evaluate(self):
        dev = DataLoader(self.dev, batch_size=self.batch_size, shuffle=False, collate_fn=self.train.collate_fn)

        batch_losses = []
        for batch in tqdm(dev):
            batch_loss = self._evaluate_partial(batch)
            batch_losses.append(batch_loss)
        batch_losses = pd.DataFrame(batch_losses)
        avg_nll = np.mean(batch_losses['nll'])
        avg_zkl_real = np.mean(batch_losses['zkl_real'])
        self.logger.info("eval avg nll: "+str(avg_nll)+' eval avg zkl real: '+str(avg_zkl_real))
        tot_eval_loss = avg_nll + avg_zkl_real 
        self.logger.info('current eval loss: '+str(tot_eval_loss)+' global step is '+str(self.global_step))
