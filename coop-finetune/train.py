...
class Trainer:
    def __init__(self,
                 ...
                 max_norm: float = 1.,
                 num_keep: int = 10,
                 pretrained_optimus = None,
                 forced_beta_during_finetune = None
                 ):
        log_dir = Path(log_dir)
        

        if pretrained_optimus is not None:
            print('[warining:] trainer.py: pretrained optimus being used, from')
            print(pretrained_optimus)
            self.model = VAE(pretrained_optimus).model 
            print('... model loaded')
            if forced_beta_during_finetune:
                print('[warining:] setting beta of vae to ', forced_beta_during_finetune)
                self.model.beta = forced_beta_during_finetune
        else:
            self.model = model
